                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    <button wire:click="toggleStatus({{ $service->id }})"
                                        class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-white {{ $service->is_active ? 'bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700' : 'bg-gradient-to-r from-gray-400 to-gray-500 hover:from-gray-500 hover:to-gray-600' }} rounded-lg shadow-md hover:shadow-lg transition-all duration-200 transform hover:scale-105"
...
@push('scripts')
    <script>
        window.addEventListener('notify', event => {
            ...
            toast.className =
                `fixed top-4 right-4 px-6 py-4 rounded-lg shadow-lg text-white ${event.detail.type === 'error' ? 'bg-red-500' : 'bg-green-500'}`;
