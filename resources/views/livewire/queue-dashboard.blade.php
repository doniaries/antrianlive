        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 transition-colors duration-200">
                <div class="text-center">
                    <div class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $statistics['total_today'] }}
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 transition-colors duration-200">
                <div class="text-center">
                    <div class="text-2xl font-bold text-yellow-600 dark:text-yellow-400">{{ $statistics['waiting'] }}
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 transition-colors duration-200">
                <div class="text-center">
                    <div class="text-2xl font-bold text-orange-600 dark:text-orange-400">{{ $statistics['called'] }}
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 transition-colors duration-200">
                <div class="text-center">
                    <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ $statistics['finished'] }}
                    </div>
                        ...
                        @else
                            <div class="space-y-4">
                                @foreach ($calledQueues as $queue)
                                    <div
                                        class="flex items-center justify-between p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg transition-colors duration-200">
                                        <div class="flex items-center space-x-4">
                                            <div class="text-3xl font-bold text-yellow-800 dark:text-yellow-400">
                                                {{ $queue->formatted_number }}
                                            </div>
                                            <div>
                                                <div class="font-semibold text-gray-900 dark:text-yellow-100">
                                                    {{ $queue->service->name }}</div>
                                        ...
                                        <div class="text-right">
                                            <div class="text-sm text-gray-600 dark:text-yellow-300/80">
                                                {{ $queue->called_at->format('H:i:s') }}</div>
                                            <button wire:click="finishQueue({{ $queue->id }})"
                                                class="mt-1 px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-105">
                                                Selesai
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <div class="space-y-4">
                                @foreach ($services as $service)
                                    @if (isset($waitingQueues[$service->id]) && $waitingQueues[$service->id]->isNotEmpty())
                                        <div>
                                            <h3 class="font-semibold text-gray-900 dark:text-gray-200 mb-2">
                                                {{ $service->name }}</h3>
                                            <div class="flex flex-wrap gap-2">
                                                @foreach ($waitingQueues[$service->id]->take(5) as $queue)
                                                    <div
                                                        class="px-3 py-2 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 rounded text-sm font-medium transition-colors duration-200">
                                                        {{ $queue->formatted_number }}
                                                    </div>
                                                @endforeach
                                                @if ($waitingQueues[$service->id]->count() > 5)
                                                    <div
                                                        class="px-3 py-2 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded text-sm transition-colors duration-200">
            ...
            <div>
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow transition-colors duration-200">
                    ...
                    <div class="p-4">
                        @foreach ($services as $service)
                            <div
                                class="mb-4 p-3 border border-gray-200 dark:border-gray-700 rounded-lg transition-colors duration-200">
                                <h3 class="font-semibold text-gray-900 dark:text-gray-200 mb-2">{{ $service->name }}
                                </h3>
                                <div class="text-sm text-gray-600 dark:text-gray-400 mb-2">
                                    Menunggu:
                                    {{ isset($waitingQueues[$service->id]) ? $waitingQueues[$service->id]->count() : 0 }}
                                </div>
                                ...
                                @if ($nextQueue)
                                    <div class="mb-2">
                                        ...
                                        <div class="text-lg font-bold text-blue-600 dark:text-blue-400">
                                            {{ $nextQueue->formatted_number }}</div>
                                    </div>
                                @endif
                ...
                <div class="mt-4 bg-white dark:bg-gray-800 rounded-lg shadow transition-colors duration-200">
                    ...
                    <div class="p-4 space-y-2">
                        <button wire:click="$refresh"
                            class="w-full px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-gray-500 to-gray-600 hover:from-gray-600 hover:to-gray-700 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                            wire:loading.attr="disabled" wire:loading.class="opacity-50 cursor-not-allowed">
                            <div class="flex items-center justify-center">
                                <svg wire:loading wire:target="$refresh"
                        ...
                        <a href="{{ route('antrians.index') }}"
                            class="w-full px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-105 text-center block">
                            Kelola Antrian
                        </a>
                    </div>
                </div>
